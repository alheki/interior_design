...
extension StringExtension on String {
  ...
  String toCamelCase() {
    ...
    final rest = words.skip(1).map((w) => '${w[0].toUpperCase()}${w.substring(1).toLowerCase()}');
  ...
  DateTime? toDateTime({String format = 'yyyy-MM-ddTHH:mm:ss'}) {
    ...
    return DateTime.tryParse(replaceAll(RegExp(r'\.\d+'), ''), );
  }
  ...
  bool get startsWithUpper=>  isNotEmpty == true && RegExp(r'^[A-Z]').hasMatch(this);



  ///returns true if the string contains only digits,
  ///otherwise false. If the input is null or empty, returns false
  bool get containsOnlyDigits => isNotEmpty == true && RegExp(r'^\d+$').hasMatch(this);
  ...
  bool isUUID() {
    return isNotEmpty == true &&
        RegExp(r'^[a-f\d]{8}-[a-f\d]{4}-[a-f\d]{4}-[a-f\d]{4}-[a-f\d]{12}$', caseSensitive: false).hasMatch(this);
  }
